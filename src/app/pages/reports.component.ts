...
@Component({
  ...
  styles: [`
    ...
    .mr-add-row input, .mr-add-row select {
      ...
      border-radius: 8px; padding: 8px 10px; font-size: 14px; color: var(--brand-label);
      outline: none;
    }
    .mr-add-row button {
      padding: 11px 22px; border-radius: 22px; border: none; cursor: pointer;
      font-size: 15px; font-weight: 600; background: var(--brand-gold-grad); color: #fff;
      flex-shrink: 0;
    }
    .mr-add-row button:disabled { opacity: 0.4; cursor: not-allowed; }
    .mr-divider { position: absolute; bottom: 0; left: 33px; right: 0; height: 0.5px; background: var(--brand-sep); }
  `],
  template: `
    <div data-testid="rules-page">

      <sp-top-bar title="Moderation rules" [large]="true" [showBack]="true" (onBack)="goBack()">
      ...
      <ng-container *ngIf="!isLoading && !loadError">
        ...
        <div style="margin:0 16px 16px;background:rgba(79,143,95,0.12);border-radius:14px;padding:14px;display:flex;align-items:center;gap:12px">
          ...
          <div style="flex:1;min-width:0">
            <div style="font-size:15px;font-weight:600;color:var(--brand-label)">Engine running</div>
            <div style="font-size:13px;color:var(--brand-label-secondary)">
              {{ activeRules }} of {{ rules.length }} rules active · {{ pending }} pending posts
            </div>
          </div>
          <m-icon name="chev-r" [size]="16" color="var(--brand-label-tertiary)"></m-icon>
        </div>

        <!-- Active rules -->
        <sp-sett-group-header action="Edit">Active rules</sp-sett-group-header>
        <sp-sett-group>
          <div *ngFor="let rule of rules; let last = last" class="mr-rule-row"
            ...
            <div style="flex:1;min-width:0">
              <div style="font-size:16px;font-weight:500;color:var(--brand-label)">{{ rule.keyword }}</div>
              <div style="font-size:13px;color:var(--brand-label-secondary)">
                {{ rule.riskLevel }} · {{ rule.active ? 'Active' : 'Inactive' }}
              </div>
            </div>
            <sp-toggle [on]="rule.active" (onChange)="toggle(rule.id)"></sp-toggle>
            <div *ngIf="!last" class="mr-divider"></div>
          </div>

          <div *ngIf="!rules.length" style="padding:20px 16px;text-align:center;color:var(--brand-label-secondary);font-size:14px">
            No rules yet — add the first keyword below
          </div>
        </sp-sett-group>

        <!-- Add rule form -->
        <sp-sett-group-header>Add keyword rule</sp-sett-group-header>
        <sp-sett-group>
