import { Component, inject } from '@angular/core';
import { Router } from '@angular/router';
import { TopBarComponent } from '../design-system';

@Component({
  standalone: true,
  imports: [TopBarComponent],
  template: `
    <div style="background:#F3F1ED;min-height:100%;display:flex;flex-direction:column">
      <sp-top-bar title="Composer" [showBack]="true" (onBack)="back()"></sp-top-bar>
    </div>
  `,
})
export class SettingsComposerComponent {
  // TODO(backend): wire to composer preferences endpoint when available
  // UI is rendered with local state only until then
  private readonly router = inject(Router);
  back(): void { this.router.navigateByUrl('/settings'); }
}
